...
namespace ReportGenerator
{
    ...
    { 
        ...
        private static void HandleMessageReceived(object sender, string args)
        {
            ...
            Thread xmlThread = new Thread(() => ToXML.ConvertToXML(args, "Generated-Report", filepath));
            xmlThread.Start();

            Thread csvThread = new Thread(() => ToCSV.ConvertToCSV(args, "Generated-Report", filepath));
            csvThread.Start();

            Thread xlsxThread = new Thread(() => ToXLSX.ConvertToXLSX(args, "Generated-Report", filepath));
            xlsxThread.Start();

            Thread jsonThread = new Thread(() => ToJSON.ConvertToJSON(args, "Generated-Report", filepath));
            jsonThread.Start();

            Thread xltTread = new Thread(() => ToXLT.ConvertToXLT(args, "Generated-Report", filepath));
            xltTread.Start();

            Thread xlsmThread = new Thread(() => ToXLSM.ConvertToXLSM(args, "Generated-Report", filepath));
            xlsmThread.Start();
        }
    }
}